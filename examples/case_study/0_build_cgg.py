from examples.case_study import folder_name
...
hold_recognized_object = PromptReaction(ap["object_recognized"] & ~ap["hold"], ap["hold"] & ~ap["drop"])
# hold_recognized_object = PromptReaction(ap["object_recognized"] & ~ap["hold"], ap["hold"])
keep_holding_until_garbage = ConditionalWait(ap["hold"] & ~ap["drop"], ap["k3"])
...
run = cgg.orchestrate(n_steps=50)
print(run)