...
from core.specification.formula.patterns.robotics.coremovement.surveillance import (
    StrictOrderedPatrolling,
)
from core.specification.formula.patterns.robotics.trigger.triggers import (
    BoundDelay,
    BoundReaction,
)
...
try:
    ...
    n_day = Node(
        name="day_patrol_a", context=w["day"], specification=ordered_patrol_day, world=w
    )
    ...
    n_register = Node(
        name="register_person", context=w["day"], specification=register, world=w
    )
    ...
    Store.save_to_file(
        f"{res}", f"results.txt", absolute_folder_path=f"{result_folder}"
    )

    """Save simulation as text file"""
    Store.save_to_file(
        str(run), file_name="run.txt", absolute_folder_path=f"{result_folder}"
    )
