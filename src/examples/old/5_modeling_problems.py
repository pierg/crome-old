...
from core.specification.atom.pattern.basic import G, Init
...
try:
    ...
    print(
        "The CGG is realizable but it does not reflect the behaviour that we want"
...
try:
    ...
    n1 = Node(
        name="awake_during_day", specification=Contract(guarantees=G(day >> awake_now))
    )
...
try:
    ...
    n1 = Node(
        name="awake_during_day", specification=Contract(guarantees=G(day >> office))
    )

    n2 = Node(
        name="sleep_during_night", specification=Contract(guarantees=G(night >> bed))
    )
    ...
    print(
        "Although its satisfiable and realizable it does not reflect what we want i.e.:"
