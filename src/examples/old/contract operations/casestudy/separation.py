...
print("let us now try to refine 'original_goal' from 'library_1'")

library_1_goal_z = Node(
...
print(
    "we found a goal in library 1 which has some common variables but does not refine 'strict_patrolling'"
)
assert set(library_1_goal_z.specification.typeset).intersection(
    original.specification.typeset
)
assert not library_1_goal_z.specification <= original.specification

print("let's use the 'QUOTIENT' to look at the missing part")
quotient_original = original.quotient(library_1_goal_z)
...
print(
    "let's compute the separation on what we have a find the smallest change we have to add to our specification "
    "so that the goal we have is enough to refine"
)

separation_1 = library_1_goal_z.separation(original)
print(separation_1)
...
print(
    "thanks to the separation and the merging the initial specification has been 'relaxed' "
    "to accommodate what is available in the library"
)
assert original.specification <= merger.specification

print(
    "we can go back to the original 'original_goal' specification my separating "
