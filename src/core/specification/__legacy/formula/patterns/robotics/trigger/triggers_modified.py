...
class InstantaneousReaction(Trigger):
    ...
    def __init__(
        ...
        new_typeset, pre, post, context, active = Trigger.process_bin_contextual_input(
            pre, post, context, active
        )
...
class PromptReaction(Trigger):
    ...
    def __init__(
        ...
        f = Logic.g_(
            Logic.implies_(
                Logic.and_([c, pre]),
                Logic.x_(Logic.w_(Logic.not_(c), Logic.and_([c, post]))),
            )
        )
...
class DelayedReaction(Trigger):
    ...
    def __init__(
        ...
        c = Logic.and_([context, active])
        f = Logic.g_(
            Logic.implies_(Logic.and_([c, pre]), Logic.f_(Logic.and_([c, post])))
        )
...
if __name__ == "__main__":
    print(
        BoundReaction(
            Boolean("person"), Boolean("greet"), Boolean("active"), Boolean("true")
        )
    )
...
class Wait(Trigger):
    ...
    def __init__(
        ...
        c = Logic.and_([context, active])
        f = Logic.u_(
            Logic.or_([Logic.and_([c, pre]), Logic.not_(c)]), Logic.and_([c, post])
        )
        super().__init__(formula=(f, new_typeset))
